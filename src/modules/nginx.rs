...
#[async_trait]
impl SetupModule for Nginx {
    fn id(&self) -> ModuleId { ModuleId::Nginx }
    fn name(&self) -> &'static str { "NGINX" }
    fn description(&self) -> &'static str { "High-performance reverse proxy and web server" }
    fn dependencies(&self) -> Vec<ModuleId> { vec![] }
    fn conflicts(&self) -> Vec<ModuleId> { vec![ModuleId::Caddy, ModuleId::Traefik] }
    ...
    async fn preflight(&self, _ctx: &Context) -> ModuleResult<PreflightResult> {
        if which::which("nginx").is_ok() {
            return Ok(PreflightResult::Warning("NGINX is already installed".into()));
        }
